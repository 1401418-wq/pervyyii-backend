...
-- ysclid — метка перехода из ПОИСКА Яндекса, не рекламного клика. Загрузчик v7 кладёт её
-- в то же поле, что и yclid; в Yclid такую строку слать нельзя (Метрика отвергает загрузку),
-- но и выбрасывать жаль: это признак, что человек пришёл из органики.
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS ysclid TEXT;
