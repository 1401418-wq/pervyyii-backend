(function () {
  ...
  // Атрибуция заявки к рекламному клику. Браузерная цель доходит не всегда: 28.07
  // заявка по цирку-шапито пропала, потому что у человека вообще не отработала Метрика.
  // yclid лежит в адресе рекламного перехода и от Метрики не зависит — его и запоминаем
  // на весь визит, чтобы сервер потом сам сообщил о заявке офлайн-конверсией.
  var PT_ATTR_KEY = "pt_attr_yclid";
  var ptYclid = "";
  try {
    var q = new URLSearchParams(location.search);
    var fromUrl = q.get("yclid") || q.get("ysclid") || "";
    if (fromUrl) sessionStorage.setItem(PT_ATTR_KEY, fromUrl.slice(0, 64));
    ptYclid = (sessionStorage.getItem(PT_ATTR_KEY) || "").slice(0, 64);
  } catch (_) {}

  var ptClientId = "";
  try {
    if (typeof window.ym === "function") {
      window.ym(PT_COUNTER, "getClientID", function (id) {
        ptClientId = String(id || "").slice(0, 64);
        sendAttr();
      });
    }
  } catch (_) {}

  // Атрибуцию считаем доставленной только по подтверждению из iframe: порядок
  // сообщений не гарантирован, и отправка «в один конец» терялась, если iframe
  // ещё не успел навесить обработчик.
  var attrAcked = false;
  var attrTries = 0;
  function sendAttr() {
    if (attrAcked || !iframe || !iframe.contentWindow) return;
    if (!ptYclid && !ptClientId) return;
    if (attrTries >= 3) return;
    attrTries++;
    try {
      iframe.contentWindow.postMessage(
        {__ptAttr: {yclid: ptYclid, clientId: ptClientId}}, ALLOWED_ORIGIN);
    } catch (_) {}
    setTimeout(function () { if (!attrAcked) sendAttr(); }, 1200);
  }
  ...
  window.addEventListener("message", function (e) {
    ...
    if (e.data.__ptAttrAck) attrAcked = true;
    if (e.data.__ptReady) { attrTries = 0; sendAttr(); }
  });
